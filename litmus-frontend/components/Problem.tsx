...
const failures = [
  ...
  { k: "destabilize logits", d: "4-bit pushes fragile architectures into instability." },
  { k: "save almost nothing", d: "Sometimes the VRAM you reclaim isn't worth the tax." },
];
    ...
    <section id="problem" className="relative mx-auto max-w-6xl px-5 py-28 sm:px-8">
      ...
      <Reveal delay={0.1}>
        <p className="mt-6 max-w-2xl text-lg text-bone-muted">
          Memory reduction alone doesn&apos;t determine deployment quality. The same quantized model
          can quietly:
        </p>
      </Reveal>
      ...
      <Reveal delay={0.1}>
        <p className="mx-auto mt-12 max-w-2xl text-center text-lg text-bone">
          litmus-lab exists to <span className="text-coral">mathematically</span> decide whether
          quantization is actually worth deploying — on{" "}
          <span className="underline decoration-coral/50 underline-offset-4">your</span> hardware.
        </p>
      </Reveal>
    </section>
  );
